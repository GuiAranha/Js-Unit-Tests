...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contedos os valores esperados', () => {
    ...
    //expect([3, 28.25, 18.84]).toEqual(expect.arrayContaining(Object.values(circle(3))));
    myValues = [3, 28.25, 18.84];
    circleValues = Object.values(circle(3));
    for (let i = 0; i < circleValues.length; i += 1) {
      expect(myValues[i]).toBeCloseTo(circleValues[i], 1);
    }
  });
});
